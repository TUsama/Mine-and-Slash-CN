...
public class IdentifyTomePlusItem extends Item implements IShapedRecipe {
    public IdentifyTomePlusItem() {
    ...
    private void spawnEffects(LivingEntity en) {
        ...
        SoundUtils.playSound(en, SoundEvents.BLOCK_ENCHANTMENT_TABLE_USE, 1, 1.25F);
    }

    @Override
    @OnlyIn(Dist.CLIENT)
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(Words.RightClickToIdentifyAll.locName());
    }

    @Override
    public ShapedRecipeBuilder getRecipe() {
        return shaped(ModItems.IDENTIFY_TOME_PLUS.get(), 8)
            .key('b', Items.BOOK)
            .key('v', Items.COAL)
